...
class TestUtility:
    ...
    # returns list of sessions, 1 by default
    @classmethod
    def get_sessions(cls, amount=1, timeout=2, is_visible=True):
        if amount == 1:
            return Util.get_web_driver(wait_time=timeout,withUI=is_visible)
        sessions = []
        for i in range(amount):
            sessions.append(Util.get_web_driver(wait_time=timeout,withUI=is_visible))
        return sessions
    ...
    def test_with_pyqt_slots(cls, outer_signals, pages: [SubjectPage] = None, test_key='test_result'):
        ...
        if pages is None:
            try:
                pages_collection = util.get_pages()
