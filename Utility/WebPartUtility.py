...
class WebPartUtility_:
    ...
    def set_press_releases(cls, page: SubjectPage, session: webdriver.Chrome):
        ...
        try:
            ...
            PageUtility.set_link_status(cur_link)
            if not cur_link.status_code == 200:
                page.press_releases.errors.append(Error(error_type=ErrorType.TO_ALL_MASSAGES_IS_BROKEN.value,wp_type=type))
        except TimeoutException:
            return
        except NoSuchElementException:
            page.press_releases.errors.append(Error(error_type=ErrorType.TO_ALL_MASSAGES_IS_MISSING.value,wp_type=type))
            return
