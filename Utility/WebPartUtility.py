...
from CbsObjects.WebParts import Type as WpType
...
class PageUtility:
    ...
    def link_state(cls, element, attrib, link_list, errors, iteration):
        ...
        if not link.status_code == 200:
            ...
            if attrib == 'href':
                e_type = ErrorType.BROKEN_LINK.value
            else:
                e_type = ErrorType.BROKEN_IMAGE_LINK.value
            errors.append(Error(error_type=e_type, index=iteration + 1))
...
class WebPartUtility:
    ...
    def set_heb_statistical(cls, page: SubjectPage, session: WebDriver):
        ...
        try:
            ...
            if not title == 'עלוני סטטיסטיקל':
                print('title in stats is: ', title)
                page.stats_part.errors.append(Error(error_type=ErrorType.TITLE_NOT_CORRECT.value, wp_type=type))
            log_test += 1
        except NoSuchElementException:
            page.stats_part.errors.append(Error(error_type=ErrorType.TITLE_NOT_CORRECT.value, wp_type=type))
            print('stats not contain any title: ', page.name)
        ...
        # print('num images: {}, num links: {}'.format(len(images),len(links)))
        if len(images) == 0:
            page.stats_part.errors.append(Error(error_type=ErrorType.MISSING_IMAGES.value, wp_type=type))
        else:
            images, errors = PageUtility.set_url_links(images, attrib='src')
            for error in errors:
                error.wp_type = type
        ...
        if len(links) == 0:
            page.stats_part.errors.append(Error(error_type=ErrorType.MISSING_LINKS.value, wp_type=type))
        else:
            links, errors = PageUtility.set_url_links(links)
            for error in errors:
                error.wp_type = type
        ...
        try:
            ...
            page.stats_part.errors.append(Error(error_type=ErrorType.TO_ALL_MASSAGES_IS_MISSING.value, wp_type=type))
            return
        # text test
        # text = all_stats_link.text
        # if not text == 'לכל עלוני הסטטיסטיקל >':
        ...
        PageUtility.set_link_status(cur_link)
        if not cur_link.status_code == 200:
            page.stats_part.errors.append(Error(error_type=ErrorType.TO_ALL_MASSAGES_IS_BROKEN.value, wp_type=type))
        print('stats ended in: {} with {} checks'.format(page.name, log_test))

    @classmethod
    def set_top_box(cls, page: SubjectPage, session: webdriver.Chrome):
        type = WpType.TOP_BOX.value
        print('top-box test in: {}'.format(page.name))
        main_element, error = cls.get_main_element('TOP_BOX_XPATH', session)
        if main_element is None:
            if error == 'chrome session error':
        ...
        if len(elements) == 0:  # inside elements are not displayed
            page.top_box.errors.append(Error(error_type=ErrorType.NO_CONTENT.value, wp_type=type))
            return
    ...
    @classmethod
    def set_more_links(cls, page: SubjectPage, session: webdriver.Chrome):
        type = WpType.ADDITIONAL_LINKS.value
        print('more-links test in: {}'.format(page.name))
        ...
        title = root_element.find_element(By.XPATH, "./h2/span").text
        if not title == 'קישורים נוספים':
            page.more_links.errors.append(Error(error_type=ErrorType.TITLE_NOT_CORRECT.value, wp_type=type))

        # inside links check
        links = root_element.find_elements(By.XPATH, "./ul/div/div/div/div/ul/li/div/div[1]/a")
        # links = list(map(lambda li: CbsLink(url=li.get_attribute('href'), page_name=li.text), links))
        if len(links) < 1:
            page.more_links.errors.append(Error(error_type=ErrorType.NO_CONTENT.value, wp_type=type))
        links, errors = PageUtility.set_url_links(links)
        for error in errors:
            error.wp_type = type
        # page.press_releases.links.extend(links)
    ...
    @classmethod
    def set_sub_subjects(cls, page: SubjectPage, session: webdriver.Chrome):
        type = WpType.SUB_SUBJECTS.value
        print('sub-subjects test in: {}'.format(page.name))
        ...
        try:
            ...
            if not title == 'נושאי משנה':
                print('title is: ', title)
                page.sub_subjects.errors.append(Error(error_type=ErrorType.TITLE_NOT_CORRECT.value, wp_type=type))
        except NoSuchElementException as e:
            print('Exception: ', e, " in sub-subject, page: ", page.name)
            page.sub_subjects.errors.append(Error(error_type=ErrorType.TITLE_NOT_CORRECT.value, wp_type=type))
        except TimeoutException as e:
        ...
        links = root_element.find_elements(By.XPATH, "./ul[@class='subtopicsList']/li/a")
        if len(links) == 0:
            page.sub_subjects.errors.append(Error(error_type=ErrorType.NO_CONTENT.value, wp_type=type))
            return
        links, errors = PageUtility.set_url_links(links)
        # errors = [error + ' ' for error in errors]
        for error in errors:
    ...
    @classmethod
    def set_press_releases(cls, page: SubjectPage, session: webdriver.Chrome):
        type = WpType.PRESS_RELEASES.value
        print('press-releases test in: {}'.format(page.name))
        ...
        try:
            ...
            title = title.text
            if not title == 'הודעות לתקשורת':
                page.press_releases.errors.append(Error(error_type=ErrorType.TITLE_NOT_CORRECT.value, wp_type=type))
        except NoSuchElementException:
            print('press_releases title fail in: ', page.name)
            page.press_releases.errors.append(Error(error_type=ErrorType.TITLE_NOT_CORRECT.value, wp_type=type))
        ...
        try:
            links = root_element.find_elements(By.XPATH, "./div[@id='Control_List_Subject_Link']/ul/li/div/div/a")
            if len(links) == 0:
                page.press_releases.errors.append(Error(error_type=ErrorType.NO_CONTENT.value, wp_type=type))
                return
            links, errors = PageUtility.set_url_links(links)
            for error in errors:
                error.wp_type = type
        ...
        try:
            ...
            PageUtility.set_link_status(cur_link)
            if not cur_link.status_code == 200:
                page.press_releases.errors.append(
                    Error(error_type=ErrorType.TO_ALL_MASSAGES_IS_BROKEN.value, wp_type=type))
        except TimeoutException:
            return
        except NoSuchElementException:
            page.press_releases.errors.append(
                Error(error_type=ErrorType.TO_ALL_MASSAGES_IS_MISSING.value, wp_type=type))
            return
    ...
    def set_publications(cls, page: SubjectPage, session: webdriver.Chrome):
        ...
        try:
            title = root_element.find_element(By.XPATH, './h2/span').text
            if not title == 'פרסומים':
                page.publications.errors.append(Error(error_type=ErrorType.TITLE_NOT_CORRECT.value, wp_type=type))
        ...
        try:
            links = root_element.find_elements(By.XPATH, './div/ul/li/div/div/a')
            if len(links) < 1:
                page.publications.errors.append(Error(error_type=ErrorType.NO_CONTENT.value, wp_type=type))
            else:
                links, errors = PageUtility.set_url_links(links)
                errors = [error + ' ' for error in errors]
                page.publications.errors.extend(errors)
        ...
        try:
            ...
            #     return
            if not link.status_code == 200:
                page.publications.errors.append(
                    Error(error_type=ErrorType.TO_ALL_MASSAGES_IS_BROKEN.value, wp_type=type))

        except Exception as e:
            print("publication last link exception {}".format(e))
            page.publications.errors.append(Error(error_type=ErrorType.TO_ALL_MASSAGES_IS_BROKEN.value, wp_type=type))
    ...
    def set_tables_and_charts(cls, page: SubjectPage, session: webdriver):
        ...
        try:
            ...
            PageUtility.set_link_status(to_all_maps)
            if not to_all_maps.status_code == 200:
                page.tables_and_charts.errors.append(
                    Error(error_type=ErrorType.TO_ALL_MASSAGES_IS_BROKEN.value, wp_type=type))
    ...
    def set_international_comparisons(cls, page: SubjectPage, session: webdriver.Chrome):
        ...
        try:
            ...
            PageUtility.set_link_status(link)
            if not link.status_code == 200:
                page.international_comparisons.errors.append(
                    Error(error_type=ErrorType.BROKEN_LINK.value, wp_type=type))
            # else:
            #     print('link is ok')
            # print('status: ', link.status_code)
        except NoSuchElementException as e:
